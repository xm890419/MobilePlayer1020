...
public class LyricParaser {
    ...
    private String analyzeLyric(String line) {
        ...
        if(pos1 ==0 && pos2 != -1){
            //解析歌词 装时间的集合
            long[] timeLongs = new long[getCuontTag(line)];
            String timeStr = line.substring(pos1+1,pos2);
            timeLongs[0] = strTimeToLong(timeStr);
            if(timeLongs[0] == -1){
                return "";
            }
            int i = 1;
            ...
            while (pos1 == 0 && pos2 != -1){
                ...
                if(pos2 != -1){//还有下一句
                    ...
                    timeLongs[i] = strTimeToLong(timeStr);
                    if(timeLongs[i] == -1){
                        return "";
                    }
                    i++;
    ...
    //毫秒 02:04.12
    private long strTimeToLong(String timeStr) {
        long time = -1;
        try {
            //1.根据":"切成02和04.12
    ...
    private int getCuontTag(String line) {
        int count = 1;
